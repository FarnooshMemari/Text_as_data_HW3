...
if len(top_neg_idx) == 0:
    print("  (No negative-weight words — L1 zeroed out all of them)")
else:
    for i in top_neg_idx:
        print(f"  {feature_names[i]:25s}  {coefs[i]:+.4f}")
    if len(top_neg_idx) < 15:
        print(
            f"  (Only {len(top_neg_idx)} negative-weight words — L1 zeroed out the rest)"
        )
...
ax.set_title("L1 — Top 15 Positive-Weight Words (CORE = 1)")
plt.tight_layout()
plt.savefig("output/l1/top15_positive_words.png", dpi=150)
plt.close()

# ── Bar chart: negative words (only actual non-zero) ────────────────
if len(top_neg_idx) > 0:
    neg_words = [feature_names[i] for i in top_neg_idx]
    neg_vals = [coefs[i] for i in top_neg_idx]

    fig, ax = plt.subplots(figsize=(8, max(3, len(neg_words) * 0.4)))
    ax.barh(neg_words[::-1], neg_vals[::-1], color="#e76f51")
    ax.set_xlabel("Coefficient Weight")
    ax.set_title("L1 — Negative-Weight Words (NEG = 0)")
    plt.tight_layout()
    plt.savefig("output/l1/negative_words.png", dpi=150)
    plt.close()

# ── Save metrics to text file ───────────────────────────────────────
with open("output/l1/metrics.txt", "w") as f:
    f.write("L1 Model — Evaluation Results\n")
    f.write("=" * 50 + "\n\n")
...
print("\nAll outputs saved to output/l1/")
